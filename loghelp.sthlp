...
NB: Within the Stata help file, the line uses the {help smcl} command {cmd:{c -(}stata} {it:args}[{cmd::}{it:text}]{cmd:{c )-}};
...
{p 8 13 2}
{opt log} {cmd:using} {it:{logfilename}} {cmd:,} {opt inputname(filename)}
[{opt append} {opt replace} {opt text} {opt smcl} {opt msg} {opt vsquish}]
...
Naming behavior including the extension is the same as for Stata's {help log}.
{p_end}
...
{p_end}
{synopt:{opt append}}
specifies that results be appended to an existing file (same behavior as for Stata's {help log}).
{p_end}
{synopt:{opt replace}}
specifies that {it:filename}, if it already exists, be overwritten (same behavior as for Stata's {help log}).
{p_end}
{synopt:{opt text}}
specifies that the log file is to be in text format (same behavior as for Stata's {help log}).
{p_end}
{synopt:{opt smcl}}
specifies that the log file is to be in smcl format (same behavior as for Stata's {help log}).
{p_end}
{synopt:{opt msg}}
requests the usual suppresses the default message displayed at the top and bottom of the log file
({b:opposite} behavior vs Stata's {help log}).
{p_end}
{synopt:{opt vsquish}}
